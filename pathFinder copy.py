    ...
    def a_star_search(self) -> None:
        ...
        while not all(drone.finished for drone in drones):
            ...
            all_zones = (
                ...
                [z for z in self.graph.zones.values()] +
                [self.graph.end_zone]
            )
            z_counts = {}
            for z in all_zones:
                c = sum(
                    1 for d in drones
                    if d.current_zone == z
                    and d.id not in in_transit
                )
                z_counts[z.name] = (c, z.max_drones)

            c_counts = {}
            for conn in self.graph.connections.values():
                e = (conn.zone_a.name, conn.zone_b.name)
                c = edge_usage.get(e, 0)
                if c > 0:
                    c_counts[f"{e[0]}-{e[1]}"] = (
                        c, conn.max_link_capacity
                    )

            self.tracking[turn] = {"zones": z_counts, "connections": c_counts}
    ...
    def generate_output(self) -> None:
        ...
        for t in range(1, max_t + 1):
            if t in all_moves:
                ...
                for name, (c, cap) in self.tracking[t]["zones"].items():
                    print(f"    {name}: {c}/{cap}")

                for name, (c, cap) in self.tracking[t]["connections"].items():
                    print(f"    {name}: {c}/{cap}")